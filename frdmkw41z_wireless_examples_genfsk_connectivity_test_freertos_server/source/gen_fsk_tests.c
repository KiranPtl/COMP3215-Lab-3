...
*****************************Packet error rate*******************************
****************************************************************************/
bool_t CT_PacketErrorRate(ct_event_t evType, void* pAssociatedValue, uint8_t addr)
{
...
//    bReturnFromSM = CT_PacketErrorRateTx(evType, pAssociatedValue, currentTest != 'T', addr);
                                   ...
                                   bool_t bReInit)
{
    ...
    static uint8_t count;			//ID number of packet.
    static uint8_t prevCount;		//ID number of last packet received.
    static uint8_t addr;			//Address packet sent to.
    ...
    if(gCtEvtRxDone_c == evType || gCtEvtRxFailed_c == evType || gCtEvtSeqTimeout_c == evType) {
    	...
    	if (gCtEvtRxDone_c == evType) {
                ...
                {
                	count = gRxPacket.payload[0];
                	addr = gRxPacket.payload[1];

                    u16ReceivedPackets++;
                    i32RssiSum += (int8_t)(pIndicationInfo->rssi);

                    //If the packet with this ID hasn't been received before...
                    if(count != prevCount)
                    {
                    	//If the packet has our address, toggle the LED.
						if (addr == myAddr)
						{
							Led3Toggle();
						}
						...
						{
							GENFSK_StartTx(mAppGenfskId, gRxBuffer, gGenFskDefaultMaxBufferSize_c + crcConfig.crcSize, 0);
//							CT_PacketErrorRateTx(gCtEvtTxDone_c, gCtEvtSelfEvent_c, 0, addr, count);
						}
                    }
...
********************************************************************************** */
bool_t CT_PacketErrorRateTx(ct_event_t evType,
                                   void* pAssociatedValue, 
                                   bool_t bReInit,
								   uint8_t addr,
								   uint8_t count)
...
//        u16SentPackets = 20;
        ...
        gTxPacket.payload[0] = count;
        gTxPacket.payload[1] = addr;
        gTxPacket.payload[2] = 0;
        gTxPacket.payload[3] = 0;
        ...
        {
            GENFSK_AbortAll();
            Serial_Print(mAppSerId, cu8PerTxMessages[2], gNoBlock_d);
            perTxState = gPerTxStateIdle_c;
        }
//        u16SentPackets++;
        Serial_Print(mAppSerId, cu8PerTxMessages[3], gNoBlock_d);
...
//            } else {
               ...
               {
                    GENFSK_AbortAll();
                    Serial_Print(mAppSerId, cu8PerTxMessages[5], gNoBlock_d);
//                    perTxState = gPerTxStateIdle_c;
               }
//
//                u16SentPackets++;
